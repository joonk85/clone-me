          ...
          <div style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 12, fontSize: 11, color: "var(--tx3)", fontFamily: "var(--mo)" }}>
                <span style={{ color: "var(--go)" }}>★ {clone.rating}</span>
                ...
                .map((qa, i) => (
                  <div key={i} style={{ marginBottom: 18, animation: "fu 0.3s ease" }}>
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginBottom: 8 }}>
                      ...
                      <div style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--sf2)", border: "1px solid var(--br)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0, alignSelf: "flex-end" }}>👤</div>
                    </div>
              ...
              <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
                <div style={{ textAlign: "center", flexShrink: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 2, justifyContent: "center", margin: "4px 0" }}>
                    {[1, 2, 3, 4, 5].map((n) => (
                      <span key={n} style={{ fontSize: 14, color: n <= Math.round(Number(avgRating)) ? "var(--go)" : "var(--sf3)" }}>
                        ★
                      </span>
                    ))}
                  </div>
                ...
                <div style={{ flex: 1 }}>
                  {dist.map(({ r, n }) => (
                    <div key={r} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 11, color: "var(--go)" }}>★</span>
              ...
              {publicFeedbacks.map((fb) => (
                <Cd key={fb.id} style={{ padding: "13px 15px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <div style={{ display: "flex", gap: 3 }}>{[1, 2, 3, 4, 5].map((n) => <span key={n} style={{ fontSize: 13, color: n <= fb.rating ? "var(--go)" : "var(--sf3)" }}>★</span>)}</div>
                  ...
                  {isSub && (
                    <>
                      <div style={{ display: "flex", gap: 7, marginBottom: 10 }}>
                        {[1, 2, 3, 4, 5].map((n) => (
                          <button type="button" key={n} onClick={() => setFbRating(n)} style={{ flex: 1, padding: "10px 4px", borderRadius: 9, border: `1px solid ${fbRating >= n ? "var(--go)" : "var(--br)"}`, background: fbRating >= n ? "var(--tg-go-bg)" : "var(--sf2)", fontSize: 20, cursor: "pointer", transition: "all 0.1s" }}>
                            ★
                          </button>
                        ))}
                      </div>
